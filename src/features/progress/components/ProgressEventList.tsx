...
export function ProgressEventList({ events }: { events: ProgressStatusEvent[] }) {
    return (
            ...
            {events.map((event, index) => (
                <li key={event.id} className="progress-event">
                    <div className="progress-event__marker">
                        ...
                        {index < events.length - 1 && <span className="progress-event__line" />}
                    </div>

                    <div className="progress-event__content">
                        <p className="progress-event__message">{event.message}</p>
