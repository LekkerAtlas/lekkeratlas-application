        ...
        rules: {
          ...
          'require-jsdoc-block': {
            meta: {
              ...
              schema: [],
              messages: {
                useJsdoc: 'Use `/** ... */` for multi-line documentation comments (not `/* ... */`).',
                noLeadingBlank: 'Do not leave a blank `*` line immediately after `/**`.',
                noTrailingBlank: 'Do not leave a blank `*` line immediately before `*/`.',
                missingSpaceAfterStar: 'Add a space after `*` in JSDoc lines (use `* text`, not `*text`).',
              },
            },
            create(context) {
                ...
                Program() {
                  ...
                  for (const c of comments) {
                    ...
                    if (raw.startsWith('/*') && !raw.startsWith('/**')) {
                      context.report({
                        ...
                        fix(fixer) {
                          ...
                          return fixer.insertTextAfterRange([c.range[0], c.range[0] + 1], '*');
                        },
                      });
                      continue;
                    }
                    ...
                    if (raw.startsWith('/**')) {
                      ...
                      if (lines.length >= 3) {
                        const firstInner = lines[1].replace(/\r$/, '');
                        const lastInner = lines[lines.length - 2].replace(/\r$/, '');

                        // Enforce `* text` spacing: disallow `*text` on non-empty lines.
                        for (let i = 1; i < lines.length - 1; i++) {
                          const line = lines[i].replace(/\r$/, '');

                          if (isBlankStarLine(line)) continue;
                          if (!isMissingSpaceAfterStar(line)) continue;
                          ...
                          context.report({
                            ...
                            fix(fixer) {
                              ...
                              const updated = [...lines];
                              updated[i] = updated[i].replace(/^(\s*\*)/, '$1 ');
                              return fixer.replaceTextRange(c.range, updated.join('\n'));
                            },
                          });
            ...
            create(context) {
              return {
                ImportDeclaration(node) {
                  ...
                  const resolvedTarget = path.resolve(currentDir, importSource);
                  const srcRoot = path.resolve(process.cwd(), 'src');
                  const relativeToSrc = path.relative(srcRoot, resolvedTarget);
                  ...
                  const normalizedRelativeToSrc = relativeToSrc.split(path.sep).join('/');
